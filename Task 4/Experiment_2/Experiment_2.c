...
void timer1_init(void)
{
	...
	TCNT1H = 0x1E; 	 //Counter higher 8 bit value
	TCNT1L = 0x34;	 //Counter lower 8 bit value
...
ISR(TIMER1_OVF_vect)
{
	...
	TCNT1H = 0x1E;	//reload counter high value
	TCNT1L = 0x34; 	//reload counter low value
	counter ++;     // increment counter variable every time, the overflow occurs .

	PORTJ = PORTJ <<1 | 0x01;// Set appropriate value to PORTJ using suitable logic to get desired output.
}
